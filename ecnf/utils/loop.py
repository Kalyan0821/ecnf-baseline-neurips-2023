from typing import Callable, Tuple, Optional, NamedTuple, Any, Protocol
...
TrainingState = chex.ArrayTree
InitStateFn = Callable[[chex.PRNGKey], TrainingState]
UpdateStateFn = Callable[[TrainingState], Tuple[TrainingState, dict]]

class EvalAndPlotFn(Protocol):
    def __call__(self, state: TrainingState, key: chex.PRNGKey,
                 iteration_n: int, save: bool, plots_dir: str) -> dict:
        """
        Run evaluation and plotting using the current state.
        Args:
            state: State
            key: Source of randomness.
            iteration_n: Training iteration number.
            save: Whether to save plots.
            plots_dir: Directory for saving plots.

        Returns:
            info: Information from evaluation for logging.
        """
    ...
    if config.resume:
        ...
        if latest_cp:
            ...
            if not config.save_state_all_devices and len(jax.devices()) > 1:
                state = jax.pmap(lambda key_: state.__class__(state.params, state.opt_state, key_))(
